"""
Docstring for ece163.Modeling.VehicleGeometry
Author: Aydan Olaez (aolaez@ucsc.edu)
File containing functions for getting points for graphics using functions written in NED frame
"""
"""
...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		# Want Body to NED DCM
		body2NED = MatrixMath.transpose(Rotations.euler2DCM(yaw, pitch, roll))
		newPoints = []

		for point in self.vertices:
			point_vector = [[point[0]], [point[1]], [point[2]]]
			ned_point = MatrixMath.multiply(body2NED, point_vector)
			...
			enu_point = Rotations.ned2enu([[ned_point[0][0], ned_point[1][0], ned_point[2][0]]])

			newPoints.append([enu_point[0][0], enu_point[0][1], enu_point[0][2]])
